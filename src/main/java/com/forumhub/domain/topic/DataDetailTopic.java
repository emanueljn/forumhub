...
public record DataDetailTopic(Long id, String titulo, String mensagem, LocalDateTime dataCriacao, Long idAutor, String curso, java.util.Set<SimpleResponse> respostas, Status status) {
    ...
    public DataDetailTopic(Topic topico) {
        this(topico.getId(), topico.getTitulo(), topico.getMensagem(), topico.getDataCriacao(), topico.getIdUsuario().getId(), topico.getCurso(), topico.getResposta().stream().map(SimpleResponse::new).collect(Collectors.toSet()), topico.getStatus());
    }
